...
class Dirs {
	struct Node{
		...
		void visit_all_nodes(int& sum) {
			for (auto& child : children) {
				if (child.weight <= 100000) {
					// std::cout << "node name: " << child.name<< " weight: " << child.weight << '\n';
					sum += child.weight;
				}
				child.visit_all_nodes(sum);
			}
		}
...
public:
	auto run(const std::string& file_name){
		...
		//std::cout << "base node, weight: " << base.weight << '\n';
		int sum = 0;
		base.visit_all_nodes(sum);
		std::cout << sum;
	}
};
